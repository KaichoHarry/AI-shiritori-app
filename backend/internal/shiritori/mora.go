...
// youonKana は直前の文字と結合して1音として扱う小さい文字(DESIGN.md 1章参照)。
//   - 拗音(ゃ・ゅ・ょ)。
//   - 外来語表記で使われる小さい母音(ぁ・ぃ・ぅ・ぇ・ぉ)。「ティ」「ディ」「ファ」
//     「フィ」「フェ」「フォ」「チェ」「シェ」「ジェ」のような組み合わせは、拗音と
//     同様に直前の文字と結合して1モーラとして扱う。これを独立モーラのままにしていると
//     「カフェ」のような語の末尾が「ぇ」という他の単語がほぼ存在しない音になり、
//     相手が絶対に返せない「最強」の単語になってしまう不具合があったため修正した。
//
// 「っ」(促音)は言語学的にはこれらと別物(直前の文字と結合せず、それ自体で1モーラ)であり、
...
//
//   - 拗音(ゃ・ゅ・ょ)や外来語表記の小さい母音(ぁ・ぃ・ぅ・ぇ・ぉ)は直前の文字と
//     結合して1モーラとして扱う(youonKana参照)。
